...
public class DreamWorldController {
    ...
    @GetMapping("/getFavoriteDreams")
    public Result getFavoriteDreams(@RequestParam Integer userID){
        log.info("获取该用户收藏的所有梦境");
        ...
        for(Integer dreamId : favoriteDreamIDs){
            Dream dream = dreamWorldService.getDreamByDreamId(dreamId);
            if(dream != null){
                dreams.add(dream);
            }
        }
    ...
    public Result updateDream(@RequestParam("dreamStatus") Integer dreamStatus,@RequestParam("dreamContent") String dreamContent,@RequestParam("dreamID") Integer dreamID){
